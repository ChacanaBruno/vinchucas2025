...
public class Ubicacion {
	...
	/**
	 * Calcula la distancia en kilómetros entre esta Ubicacion y otra Ubicacion
...
//		return redondeado;
		...
		// esta, en cambio, si contempla la curvatura y eso es importante porque se esta
		// trabajando con latitud y longitud
		// OBSERVACION: si se elige redondear, asegurarse de que las coordenadas que se
		// le pasan por parametro sean exactas, sino se aleja demasiado...
		...
		double a = Math.pow(Math.sin(dLat / 2), 2) + Math.cos(Math.toRadians(latitud1))
				* Math.cos(Math.toRadians(latitud2)) * Math.pow(Math.sin(dLon / 2), 2);
...
//	    return Math.round(distancia * 100.0) / 100.0; // redondeo a 2 decimales para que el numero no sea enorme
	...
	public List<Ubicacion> ubicacionesAMenosDeXKilometros(double km, List<Ubicacion> us) {
		return us.stream().filter(u -> (this.distanciaALaUbicacion(u)) <= km).collect(Collectors.toList());
	}

	public List<Muestra> muestrasAMenosDeXKilometros(double km, List<Muestra> ms) {
		return ms.stream().filter(m -> (this.distanciaALaUbicacion(m.getUbicacion())) <= km)
